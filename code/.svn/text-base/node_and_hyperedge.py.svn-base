...
FLAGS=flags.FLAGS
flags.DEFINE_boolean("duplicate", False, "allow duplicate trees in k-best list")
flags.DEFINE_boolean("warn_duplicate", False, "print warnings on duplicates")
flags.DEFINE_boolean("canonical", False, "only print trees from canonical action sequences")
...
class Node(Tree):
    ...
    def prepare_kbest(self):
        ...
        self.kset = set()
        self.fixed = False
        if self.is_terminal() and self.bestres is not None:
...
##      return Tree(self.label, self.span, wrd=self.word, sym=False)
    ...
    def bestparse(self, weights=FVector("0=-1"), dep=0):
        ...
        if self._terminal:
            ...
            self.besttree = self.bestedge.assemble(best_subtrees)
    ...
    def lazykbest(self, k, dep=0):
        ...
        while now < k:
            ...
            if (FLAGS.duplicate or (tree not in self.kset)):
##               and (not FLAGS.canonical or tree.canonical):
                ...
                if FLAGS.warn_duplicate:
                    print >> logs, "*** duplicate %f %s %d" % (score, tree, now)
...
##            action = 2 if left.headidx == int(self.label) else 1  # wrong
            t = left.combine(right, self.action)
            t.canonical = left.canonical and right.canonical and (self.action == 2 or right.rights == [])
        return t
    ...
    def __init__(self, head, tails, fvector):
        ...
        self.subs = tails
        self.fvector = fvector
        self.action = fvector["dir"] # 1: left, 2: right, 0: root (absent)
