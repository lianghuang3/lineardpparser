...
class DepVal(object):
    ...
    def __init__(self, yes=0, tot=1e-10, rootyes=0, roottot=1e-10, completeyes=0):
        self.yes = yes
        self.tot = tot
    ...
    def nonroot(self):
        return (self.yes - self.rootyes) / (self.tot - self.roottot) if self.tot != 0 else 0
...
class DepTree(object):
    ...
    def setup():
        if FLAGS.integerize:
            DepTree.puncs = map(Vocab.str2id, DepTree.puncs)
    ...
    def __str__(self, wtag=True):
        ...
        word_tag = self.head()
        if FLAGS.integerize:
            word_tag = tuple(map(Vocab.id2str, word_tag))
        word = word_tag[0]
            
        return "(%s)" % " ".join([x.__str__(wtag) for x in self.lefts] + \
                                 ["%s/%s" % tuple(word_tag) if wtag else word] + \
    ...
    def _parse(line, index, sent):
        ...
        while line[index] != ')':
            if line[index] == '(':
                ...
                if FLAGS.integerize:
                    head = tuple(map(Vocab.str2id, head)) # (word, tag) integer_id pair
