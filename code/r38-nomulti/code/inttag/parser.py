...
class Parser(object):
    ...
    def try_parse(self, words, refseq=None, early_stop=False, gold_tags=None):
        ''' returns myseq, mytree, goodfeats, badfeats'''

        self.State.setup()
        self.State.words = map(self.model.map_unk_wc, words) # yang, lhuang

        # lhuang
        if FLAGS.use_gold_tags:
            self.State.gold_tags = gold_tags
        self.State.early_stop = early_stop # yang
        
        n = len(words) # yang

        if FLAGS.shifttag: # yang: shift-tag
            num_steps = 2*n + FLAGS.tag_qi
        elif FLAGS.posttag: # yang: post-tag
            num_steps = 3*n
        elif FLAGS.pretag: # yang: pre-tag
            if FLAGS.tag_end_symbol:
                num_steps = 3*n + 1 + 2*FLAGS.tag_qi
                # num_steps = (2*n - 1) +  n  + (FLAGS.tag_qi + 1) + FLAGS.tag_qi + 1
                # num_steps =   basic   + tag +       tag </s>     +  shift <s>   + 1
            else:
                num_steps = 3*n + FLAGS.tag_qi
                # num_steps = (2*n - 1) +  n  + FLAGS.tag_qi + 1
                # num_steps =   basic   + tag +  shift <s>   + 1
        beams = [None] * (num_steps) #lhuang: variable seq length#(2*n)
        ...
        for i in range(1, num_steps): # 2n-1 steps

            buf = []
            gold_item = None
            for old in beams[i-1]:
                for action in old.allowed_actions():
                    # yang: action is now a pair, so is refseq
...
def work(line, i):
    ...
    if line[0]=="(":
        ...
        sentence = DepTree.words # assigned in DepTree.parse() # yang
    else:
    ...
    if FLAGS.sim is not None: # simulation, not parsing
        ...
        if True: #FLAGS.earlystop:
            ...
            gold_tags = reftree.tagseq() if FLAGS.use_gold_tags and reftree is not None else None

            tree, myseq, score = parser.try_parse(sentence, refseq, early_stop=FLAGS.earlystop, gold_tags=gold_tags)
            if FLAGS.earlystop:
    ...
    print >> logs, "sent {i:-4} (len {l}):\tmodelcost= {c:.2f}\tprec= {p:.2%}"\
          "\tstates= {ns} (uniq {uq})\tedges= {ne}\ttime= {t:.3f}\tsearched= {sp}\ttagprec= {tp:.2%}" \
          .format(i=i, l=len(sentence), c=score, p=prec.prec(), \
                  ns=nstates, uq=nuniq, ne=nedges, t=dtime, sp=searched, tp=prec.tagprec())
    if FLAGS.seq:
