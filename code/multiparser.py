...
##            action_gold = refseq is not None and action == refseq[i-1]  # 0-based
            action_gold = (action == self.refseq_action)
            for new in old.take(action, action_gold):
                buf.append(new)
                if self.early_stop and new.gold:
                    gold_item = new
        return buf, gold_item, (it, time.time())
    
    def try_parse(self, sent, refseq=None, early_stop=False, train=False):
        ...
        self.nuniq = 0
        if Parser.debuglevel >=3 and early_stop:
...
##            for old in beams[i-1]:
            self.refseq_action = refseq[i-1] if refseq is not None else None
            print >> logs, "before map", time.time(), i, len(beams[i-1])
            results = pool.map(self.take_worker, beams[i-1], chunksize=8)
            print >> logs, "after map", time.time()
            for bf, gi, (it, ot) in results:
                print >> logs, it, ot-it
                buf += bf
                if gi is not None:
                    gold_item = gi
            ...
            for j, new in enumerate(buf): # from best to worst
                if not FLAGS.dp or new not in tmp:
                    ...
                    if Parser.naive or (Parser.merge and new.action == 0): # SHIFT
                        new.rank = -2 # don't early stop!
                        if new.gold:
                            tmp[new].gold = True
                    elif Parser.debuglevel >=1 and early_stop and new.gold:
            ...
            if early_stop:
                if gold_item.rank == -1: # early stop
                    ...
                    return None, beams[i][0].all_actions(), 0, i  ## no tree, no score
                else:
                    if Parser.debuglevel >=3:                                 
        ...
        return goal.tree(), goal.all_actions(), goal.score, i
    ...
    for i, line in enumerate(shell_input(), 1):
        ...
        if FLAGS.sim is not None: # simulation, not parsing
            ...
            if True: #FLAGS.earlystop:
                ...
                tree, myseq, score, _ = parser.try_parse(sentence, refseq, early_stop=FLAGS.early)
                if FLAGS.early:
